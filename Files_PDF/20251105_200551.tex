...
% === NODOS === 
\node[NotDirectedEven] (N1) at (10,2) {1};
\node[NotDirectedEven] (N2) at (3,4) {2};
\node[NotDirectedOdd] (N3) at (5,6) {3};
\node[NotDirectedEven] (N4) at (7,1) {4};
\node[NotDirectedOdd] (N5) at (8,1) {5};
% === ARISTAS === 
\draw[<->, thick] (N1) -- (N2);
\draw[<->, thick] (N1) -- (N4);
\draw[<->, thick] (N2) -- (N5);
...
It has a hamiltonian path because there's a way to visit every vertex without repetition.\\
It's not Eulerian because at least one node has an odd degree.\\
It's semi-Eulerian because it has exactly 2 odd degree nodes.\\
\end{document}
